...
def nearcorr_batch(A, tol, n_jobs=-1):
    A = np.asarray(A)
    A = 0.5 * (A + np.transpose(A, (0, 2, 1)))  # shouldn't modify anything deeply

    # parallel just to go faster (i was scare it was to be slow be it is not actually), don't hesitate to put n_jobs = 1 or 2 (-1 is to use all of your computer's core)
    X = Parallel(n_jobs=n_jobs)(delayed(nearcorr)(A[b], tol) for b in range(A.shape[0]))
